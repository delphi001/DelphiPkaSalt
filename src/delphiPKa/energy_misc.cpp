            ...
            if (vecCrgStat[i]) {
                ...
                if (newPDB[i].res_name == "ASP") {
                    ...
                    if (crgmap.find(key) != crgmap.end()) {
                        ...
                        cout << "The charge for atom " << newPDB[i].atom_name << " in residue AS0 " << newPDB[i].res_num
                    ...
                    if (crgmap.find(key) != crgmap.end()) {
                        ...
                        cout << "The charge for atom " << newPDB[i].atom_name << " in residue LY0 " << newPDB[i].res_num
                             << " is not found. The charge will be set to 0.0 " << endl;

                        fCrg = 0.0000;
                    }
                } else if (newPDB[i].res_name == "TYR") {
                    key = "TY0 " + newPDB[i].atom_name;
                    if (crgmap.find(key) != crgmap.end()) {
                        fCrg = crgmap.find(key)->second;
                    } else {
                        cout << "The charge for atom " << newPDB[i].atom_name << " in residue TY0 " << newPDB[i].res_num
                             << " is not found. The charge will be set to 0.0 " << endl;
                ...
                } else if (newPDB[i].res_name == "CYS") {
                    key = "CY0 " + newPDB[i].atom_name;
                    if (crgmap.find(key) != crgmap.end()) {
                        fCrg = crgmap.find(key)->second;
                    } else {
                        cout << "The charge for atom " << newPDB[i].atom_name << " in residue CY0 " << newPDB[i].res_num
                             << " is not found. The charge will be set to 0.0 " << endl;
                    ...
                    if (crgmap.find(key) != crgmap.end()) {
                        ...
                        cout << "The charge for atom " << newPDB[i].atom_name << " in residue HI0 " << newPDB[i].res_num
                             << " is not found. The charge will be set to 0.0 " << endl;
                    ...
                    if (crgmap.find(key) != crgmap.end()) {
                        ...
                        cout << "The charge for atom " << newPDB[i].atom_name << " in residue AR0 " << newPDB[i].res_num
                             << " is not found. The charge will be set to 0.0 " << endl;
                    ...
                    if (crgmap.find(key) != crgmap.end()) {
                        ...
                        cout << "The charge for atom " << newPDB[i].atom_name << " in residue GL0 " << newPDB[i].res_num
                             << " is not found. The charge will be set to 0.0 " << endl;
                    ...
                    if (crgmap.find(key) != crgmap.end()) {
                        ...
                        cout << "The charge for atom " << newPDB[i].atom_name
                             << " in residue A0 is not found. The charge will be set to 0.0 "
                             << endl;
                        fCrg = 0.0000;
                    }
                    ...
                    if (crgmap.find(key) != crgmap.end()) {
                        ...
                        cout << "The charge for atom " << newPDB[i].atom_name
                             << " in residue DA0 is not found. The charge will be set to 0.0 "
                             << endl;
                        fCrg = 0.0000;
                    }
                    ...
                    if (crgmap.find(key) != crgmap.end()) {
                        ...
                        cout << "The charge for atom " << newPDB[i].atom_name
                             << " in residue C0 is not found. The charge will be set to 0.0 "
                             << endl;
                        fCrg = 0.0000;
                    }
                    ...
                    if (crgmap.find(key) != crgmap.end()) {
                        ...
                        cout << "The charge for atom " << newPDB[i].atom_name
                             << " in residue DC0 is not found. The charge will be set to 0.0 "
                             << endl;
                        fCrg = 0.0000;
                    }
                } else {
                    ...
                    if (crgmap.find(key) != crgmap.end()) {
                        ...
                        cout << "The charge for atom " << newPDB[i].atom_name << " in residue " << newPDB[i].res_name
                             << " is not found. The charge will be set to 0.0 "
                             << endl;
                        fCrg = 0.0000;
                    }
                }
...
bool CEnergy::runFocus(shared_ptr<SPrime> param, SGrid<double> &center1) {
    ...
    param->scale *= focus2initscaleratio;
    scale1 = param->scale;
    ...
    if ((gsize0 / scale0) <= (gsize1 / scale1)) {
        ...
        scale1 = param->scale;
        cout
                << " Scale = 2 The new grid will be out of box. Therefore, the scale will be set to 4.0 for next focussing run. "
                << endl;
    ...
    center0.nX = param->oldmid1.nX;
    center0.nY = param->oldmid1.nY;
    center0.nZ = param->oldmid1.nZ;
    ...
    // Check X-boundary
    dist_l = x_min0 - x_min1;
    dist_r = x_max0 - x_max1;
    if (dist_l >= 0) {
        center1.nX += dist_l + 0.01;
        bCenterReset = true;
    }
    if (dist_r <= 0) {
        center1.nX += dist_r - 0.01;
        bCenterReset = true;
    }

    // Check Y-boundary
    dist_l = y_min0 - y_min1;
    dist_r = y_max0 - y_max1;
    if (dist_l >= 0) {
        center1.nY += dist_l + 0.01;
        bCenterReset = true;
    }
    if (dist_r <= 0) {
        center1.nY += dist_r - 0.01;
        bCenterReset = true;
    }

    // Check Z-boundary
    dist_l = z_min0 - z_min1;
    dist_r = z_max0 - z_max1;
    if (dist_l >= 0) {
        center1.nZ += dist_l + 0.01;
        bCenterReset = true;
    }
    if (dist_r <= 0) {
        center1.nZ += dist_r - 0.01;
        bCenterReset = true;
    }
